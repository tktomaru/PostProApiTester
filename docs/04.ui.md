...
interface TechnicalFoundation {
  ...
  platform: {
    ...
    framework: 'TypeScript + Vanilla JavaScript';
    bundler: 'Vite';
  ...
  ui: {
    ...
    size: '800x600px（最小）～1200x800px（最大）';
  ...
  // 性能要件
  performance: {
    startup: '200ms以内'; // 拡張機能ロード時間
    uiResponse: '100ms以内'; // UI操作レスポンス
    memoryUsage: '50MB以下'; // 通常時メモリ使用量
  };
}
```
...
│ │History      ││  │  Response: [Body][Headers][Tests]  │   │
│ │ ├─ 14:30 GET││  │                                 │   │
│ │ ├─ 14:25 POST│  │  【コレクション管理画面】            │   │
│ │ └─ 14:20 PUT││  │  コレクション一覧・編集・実行制御  │   │
│ └─────────────┘│  │                                 │   │
│                │  │  【環境・変数管理画面】              │   │
│                │  │  環境切り替え・変数編集            │   │
│                │  │                                 │   │
...
interface ScreenNavigation {
  screens: {
    main: {
      path: '/';
      name: 'メイン画面';
      components: ['リクエスト作成', 'レスポンス表示', 'テスト実行'];
      primaryUse: 'API開発・テストの中心画面';
    };
    collections: {
      path: '/collections';
      name: 'コレクション管理画面';
      components: ['コレクション一覧', '詳細編集', '実行制御'];
      primaryUse: 'プロジェクト管理・整理';
    };
    environment: {
      path: '/environment';
      name: '環境・変数管理画面';
      components: ['環境切り替え', '変数編集', 'インポート・エクスポート'];
      primaryUse: '環境間での設定管理';
    };
    history: {
      path: '/history';
      name: '履歴画面';
      components: ['履歴一覧', '検索・フィルタ', '再実行'];
      primaryUse: 'デバッグ・問題解決';
    };
    settings: {
      path: '/settings';
      name: '設定画面';
      components: ['一般設定', 'セキュリティ設定', 'エクスポート'];
      primaryUse: 'アプリケーション設定';
    };
  };
  
  navigation: {
    primary: 'ヘッダーのタブナビゲーション';
    secondary: 'サイドバーのコンテキストメニュー';
    breadcrumb: '階層ナビゲーション（コレクション内）';
    shortcuts: 'キーボードショートカット対応';
  };
}
```
...
#### 4.1.3 エコーAPI連携UI設計
```typescript
interface EchoAPIIntegration {
  // エコーAPIテスト専用UI
  echoTestPanel: {
    endpoint: 'https://reply.tukutano.jp';
    description: 'API学習・検証用の安全なテスト環境';
    features: {
      quickTest: 'ワンクリックでのエコーテスト実行';
      responseInspection: 'リクエスト送信内容の詳細確認';
      specialCommands: 'エコーAPI専用テストコマンド';
    };
  };
  
  // 専用テストコマンドUI
  echoCommands: {
    'echoRequestMethodEquals': '送信メソッド検証UI';
    'echoRequestHeaderEquals': '送信ヘッダー検証UI';
    'echoRequestBodyEquals': '送信ボディ検証UI';
    'echoRequestUrlContains': '送信URL部分一致検証UI';
  };
  
  // フォールバック設計
  fallback: {
    offlineMode: 'エコーAPI利用不可時のローカルモック';
    errorHandling: 'ネットワークエラー時の適切な案内';
    alternatives: '代替テスト方法の提案';
  };
}
```
...
interface PerformanceStandards {
  // 起動性能（要件定義書 NF-001）
  startup: {
    extensionLoad: '200ms以内'; // Chrome拡張機能ロード
    uiRender: '300ms以内'; // UI初期描画
    dataLoad: '500ms以内'; // 初期データ読み込み
    measurement: 'Performance API使用';
  };
  
  // 操作レスポンス（要件定義書 NF-001）
  interaction: {
    uiResponse: '100ms以内'; // UI操作レスポンス
    inputValidation: '50ms以内'; // 入力値検証
    tabSwitch: '150ms以内'; // タブ切り替え
    measurement: 'User Timing API使用';
  };
  
  // リソース使用量（要件定義書 NF-002）
  resources: {
    memory: {
...
class UIPerformanceMonitor {
  ...
  // UI操作の性能測定
  measureUIOperation(operationName: string, operation: () => Promise<void>): Promise<void> {
  ...
  monitorMemoryUsage(): void {
    if ('memory' in performance) {
      ...
      if (usedMB > 40) { // 40MB超過で警告
        console.warn(`Memory usage: ${usedMB.toFixed(1)}MB`);
  ...
  private checkThreshold(operation: string, duration: number): void {
    ...
    if (threshold && duration > threshold) {
      console.warn(`Performance threshold exceeded: ${operation} (${duration}ms > ${threshold}ms)`);
    }
  }
}
```
...
#### 12.3.1 認証情報保護UI
```typescript
interface SecurityUIDesign {
  // 認証情報入力UI（要件定義書 NF-004準拠）
  authenticationUI: {
    basic: {
      display: 'マスク表示（●●●●●●）';
      storage: 'AES-256暗号化保存';
      validation: 'リアルタイム形式チェック';
      clearButton: '一括クリア機能';
    };
    
    bearer: {
      display: 'トークン一部表示（先頭4文字+...）';
      expiry: '有効期限警告表示';
      refresh: '自動更新UI';
    };
    
    apikey: {
      location: 'Header/Query切り替えUI';
      rotation: 'ローテーション支援';
      validation: 'キー形式チェック';
    };
  };
  ...
  // セキュリティステータス表示
  securityStatus: {
    encryption: '暗号化状態インジケーター';
    connection: 'HTTPS接続確認表示';
    certificate: 'SSL証明書状態表示';
    warnings: 'セキュリティ警告システム';
  };
}
```
...
```typescript
interface ScriptSecurityUI {
  // サンドボックス実行表示（要件定義書 NF-005準拠）
  sandboxExecution: {
    indicator: 'サンドボックス実行中表示';
    timeout: 'タイムアウト進行表示（10秒制限）';
    restrictions: '利用可能API一覧表示';
    warnings: '危険操作警告システム';
  };
  
  // コード検証UI
  codeValidation: {
    realtime: 'リアルタイム構文チェック';
    security: 'セキュリティリスク検出表示';
    suggestions: '安全な代替手段提案';
    approval: '実行前確認ダイアログ';
  };
}
```
...
✓ 技術的機能ではなく利用場面を重視した設計
  - 5画面構成による業務フロー最適化
  - コンテキスト適応型UI実装
  - 段階的複雑性提示（Progressive Disclosure）
  
✓ 具体的な業務フローに沿った機能配置
  - メイン画面：日常的なAPI開発作業
  - コレクション画面：プロジェクト管理・整理
  - 環境管理画面：設定・切り替え作業
  - 履歴画面：デバッグ・問題解決
  - 設定画面：アプリケーション管理
  
✓ ユーザーのメンタルモデルに合致したUI構造
  - APIクライアントツールの一般的な構造踏襲
  - 直感的なタブナビゲーション
  - 論理的な情報階層設計
  
✓ 実際の作業コンテキストを考慮した設計
  - 開発環境での同時作業を考慮したリソース制限
  - チーム開発での共有・協業機能
  - 緊急時のデバッグ作業効率化
```
...
✓ 価値創造循環モデルの設計への反映
  - 直感的UI→効率向上→創造時間→品質向上→ビジネス成果→投資拡大→UI改善
  - 各段階での具体的なUI支援機能設計
...
✓ 技術仕様整合性
  - Manifest V3準拠設計
  - TypeScript + Vite技術選択の反映
  - Chrome Storage API活用設計
