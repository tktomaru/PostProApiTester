...
interface DatabaseSchema {
  // メインテーブル
  requests: {
    structure: 'コレクション単位で分割保存';
    partitioning: 'collections_${collectionId}';
    indexing: ['id', 'name', 'method', 'url'];
  };
  
  collections: {
    structure: '階層構造対応';
    relationships: 'requests[]参照';
    versioning: 'データバージョン管理';
  };
  
  variables: {
    hierarchy: '3階層（global/environment/collection）';
    encryption: '機密データは暗号化';
    scope: 'アクティブ環境による解決';
  };
  
  history: {
    partitioning: 'history_${year}_${month}';
    retention: '最大1000件・90日間';
    compression: 'gzip圧縮';
  };
  
  settings: {
    storage: 'app_settings（単一キー）';
    backup: '設定変更時自動バックアップ';
  };
}
```
...
interface EchoAPIIntegration {
  ...
  // リクエスト仕様
  request: {
    anyMethod: 'GET|POST|PUT|DELETE|PATCH|HEAD|OPTIONS';
    anyPath: '任意パス対応';
    headers: '任意ヘッダー受信';
    body: '任意ボディ受信（最大10MB）';
  };
  ...
  response: {
    ...
    structure: {
      method: 'string'; // 受信HTTPメソッド
      url: 'string';    // 受信URL
      headers: 'object'; // 受信ヘッダー
      body: 'string';    // 受信ボディ（文字列化）
...
**UI設計方針**:
```typescript
interface UIDesignSpecification {
  // 画面構成
  layout: {
  ...
  inputOutput: {
    // ファイル入力
    import: {
      formats: ['JSON（独自形式）', 'Postman Collection v2.1'];
      maxSize: '50MB';
      validation: '形式チェック・エラー表示';
      dragDrop: 'ドラッグ&ドロップ対応';
    };
    
    // ファイル出力
    export: {
      formats: ['JSON（独自形式）', 'Postman Collection互換'];
      compression: 'gzip圧縮オプション';
      filtering: '選択的エクスポート';
      naming: 'タイムスタンプ付きファイル名';
    };
    
    // 帳票出力
    reports: {
      testResults: 'HTML形式テスト結果レポート';
      collectionRun: 'コレクション実行結果サマリー';
      performance: 'パフォーマンス分析レポート';
      format: 'HTML + CSS（印刷対応）';
    };
  };
}
```
...
**通信方式**:
```typescript
interface APIIntegrationDesign {
  // HTTPクライアント仕様
  httpClient: {
    implementation: 'fetch API + XMLHttpRequest併用';
    protocols: ['HTTP/1.1', 'HTTP/2', 'HTTPS'];
    methods: ['GET', 'POST', 'PUT', 'DELETE', 'PATCH', 'HEAD', 'OPTIONS'];
    
    // リクエスト処理
    request: {
      timeout: '30秒（デフォルト）～300秒（最大）';
      redirects: '自動追従（最大10回）';
      compression: 'gzip, deflate対応';
      streaming: '大容量レスポンス対応';
    };
    
    // レスポンス処理
    response: {
      maxSize: '10MB制限';
      contentTypes: ['application/json', 'text/*', 'application/xml', 'binary/*'];
      encoding: 'UTF-8自動検出';
      validation: 'Content-Type検証';
    };
  };
  ...
  security: {
    ...
    authentication: {
      basic: 'Base64エンコード';
      bearer: 'Authorizationヘッダー';
      apikey: 'Header/Query Parameter配置';
      oauth2: '将来対応（Authorization Codeフロー）';
    };
    
    // 通信セキュリティ
    transport: {
      tls: 'TLS 1.2以上必須';
      certificates: 'SSL証明書検証';
      cors: 'Chrome拡張機能権限による回避';
      csp: 'Content Security Policy準拠';
    };
  };
}
```
...
**基盤構成**:
```typescript
interface ChromeExtensionArchitecture {
  // Manifest V3構成
  manifest: {
    version: '3';
    ...
    content_security_policy: '厳格なCSP設定';
  };
  
  // アーキテクチャレイヤー
  layers: {
    presentation: {
      components: ['UI Components', 'Event Handlers', 'View Controllers'];
      technology: 'TypeScript + Vanilla DOM';
      patterns: 'MVC Pattern';
    };
    
    business: {
      components: ['Request Manager', 'Test Executor', 'Variable Manager'];
      technology: 'TypeScript Classes';
      patterns: 'Command Pattern + Observer Pattern';
    };
    
    data: {
      components: ['Storage Manager', 'Cache Manager', 'Sync Manager'];
      technology: 'Chrome Storage API';
      patterns: 'Repository Pattern';
    };
    
    infrastructure: {
      components: ['HTTP Client', 'Security Manager', 'Logger'];
      technology: 'Web APIs + Chrome APIs';
